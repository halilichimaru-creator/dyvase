...
export default function DevisClient() {
  ...
  const renderOptions = (step: QCMStep) => {
    const visibleOptions = step.showMore && !showAllOptions ? step.options.slice(0, step.initialVisible || 8) : step.options
    const hasMore = step.showMore && !showAllOptions && step.options.length > (step.initialVisible || 8)
    return (
      <>
        <div className="qcm-options">
          {visibleOptions.map(option => {
            const isSelected = step.type === 'checkbox' ? ((answers[step.id] as string[]) || []).includes(option.value) : answers[step.id] === option.value
            return (
              <motion.div
                key={option.value}
                className={`qcm-option ${isSelected ? 'selected' : ''}`}
                onClick={() => step.type === 'checkbox' ? handleCheckboxToggle(step.id, option.value) : handleRadioSelect(step.id, option.value)}
                whileHover={{ scale: 1.01 }}
                whileTap={{ scale: 0.99 }}
              >
                {step.type === 'radio' ? <div className="qcm-option__radio" /> : (
        ...
        {hasMore && <button className="qcm-show-more" onClick={() => setShowAllOptions(true)}>Afficher plus</button>}
      </>
    )
  }
  ...
  if (success) {
    return (
      <div className="devis-qcm-page">
        <motion.div className="qcm-card" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
          <div className="qcm-success">
            <motion.div className="qcm-success__icon" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: 'spring', stiffness: 200 }}>✓</motion.div>
            <h2>Demande envoyée avec succès !</h2>
            <p>Merci {contact.nom} ! Nous avons bien reçu votre demande de devis. Notre équipe analysera vos réponses et vous recontacterons sous 24h avec une proposition personnalisée.</p>
            <motion.button className="btn btn-primary" onClick={() => router.push('/')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>Retour à l'accueil</motion.button>
          </div>
        </motion.div>
      </div>
    )
  ...
  return (
    <div className="devis-qcm-page">
      <motion.div className="qcm-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="qcm-progress">
          <motion.div className="qcm-progress__bar" initial={{ width: 0 }} animate={{ width: `${progressPercent}%` }} />
        </div>
        <button className="qcm-close" onClick={() => router.push('/')} aria-label="Fermer le questionnaire">
        ...
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div key={currentStep} custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}>
            <div className="qcm-content">
              <h1 className="sr-only">Demande de Devis - Dyvase</h1>
              <div className="qcm-step-indicator">Étape {currentStep + 1} / {TOTAL_STEPS}</div>
              {isContactStep ? (
                <>
                  <h2 className="qcm-question">Vos coordonnées pour recevoir le devis</h2>
                  <div className="qcm-form">
                    {error && <motion.div className="qcm-error" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>⚠️ {error}</motion.div>}
                    <div className="qcm-form__row">
                      <div className="qcm-form__group">
                        <label className="qcm-form__label">Nom complet <span className="required">*</span></label>
                        <input type="text" className="qcm-form__input" placeholder="Jean Dupont" value={contact.nom} onChange={e => setContact(prev => ({ ...prev, nom: e.target.value }))} required />
                      </div>
                      <div className="qcm-form__group">
                        <label className="qcm-form__label">Email <span className="required">*</span></label>
                        <input type="email" className="qcm-form__input" placeholder="jean@entreprise.com" value={contact.email} onChange={e => setContact(prev => ({ ...prev, email: e.target.value }))} required />
                      </div>
                    </div>
                    <div className="qcm-form__row">
                      <div className="qcm-form__group">
                        <label className="qcm-form__label">Téléphone</label>
                        <input type="tel" className="qcm-form__input" placeholder="06 12 34 56 78" value={contact.telephone} onChange={e => setContact(prev => ({ ...prev, telephone: e.target.value }))} />
                      </div>
                      <div className="qcm-form__group">
                        <label className="qcm-form__label">Nom de l'entreprise</label>
                        <input type="text" className="qcm-form__input" placeholder="Mon Entreprise" value={contact.nom_entreprise} onChange={e => setContact(prev => ({ ...prev, nom_entreprise: e.target.value }))} />
                      </div>
                    </div>
                    <div className="qcm-form__group">
                      <label className="qcm-form__label">Message / précisions</label>
                      <textarea className="qcm-form__textarea" placeholder="Parlez-nous de votre projet, vos objectifs, vos contraintes..." rows={3} value={contact.message} onChange={e => setContact(prev => ({ ...prev, message: e.target.value }))} />
                    </div>
                  </div>
                </>
              ) : currentQCMStep ? (
            ...
            <div className={`qcm-actions ${currentStep === 0 ? 'qcm-actions--end' : ''}`}>
              {currentStep > 0 && <button className="qcm-btn-back" onClick={goBack}>Retour</button>}
              <motion.button className={`qcm-btn-next ${isContactStep ? 'qcm-btn-submit' : ''}`} onClick={goNext} disabled={!canGoNext() || submitting} whileHover={canGoNext() ? { scale: 1.03 } : {}} whileTap={canGoNext() ? { scale: 0.97 } : {}}>
                {submitting ? 'Envoi en cours...' : isContactStep ? 'Envoyer ma demande' : 'Continuer'}
              </motion.button>
            </div>
          </motion.div>
        </AnimatePresence>
